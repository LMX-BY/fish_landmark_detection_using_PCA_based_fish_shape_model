...
class LandmarkHead(nn.Module):
    def __init__(self, inchannels=512, num_anchors=3):
        ...
        self.conv1x1 = nn.Conv2d(inchannels, num_anchors * landmark_coordinate_size, kernel_size=(1, 1), stride=1,
                                 padding=0)
    ...
    def forward(self, x):
        ...
        return out.view(out.shape[0], -1, landmark_coordinate_size)


class RetinaFaceLandmarkDetectionNet3Layer(nn.Module):
    def __init__(self, cfg=None, phase='train'):
        """
        :param cfg:  Network related settings.
        :param phase: train or test.
        """
        super(RetinaFaceLandmarkDetectionNet3Layer, self).__init__()
        self.phase = phase
        backbone = None
        ...
        self.ClassHead = self._make_class_head(fpn_num=3, inchannels=cfg['out_channel'], anchor_num=cfg['prior_num_in_a_cell'])
        self.BboxHead = self._make_bbox_head(fpn_num=3, inchannels=cfg['out_channel'],anchor_num=cfg['prior_num_in_a_cell'])
        self.LandmarkHead = self._make_landmark_head(fpn_num=3, inchannels=cfg['out_channel'],anchor_num=cfg['prior_num_in_a_cell'])

    def _make_class_head(self, fpn_num=3, inchannels=64, anchor_num=16):
        classhead = nn.ModuleList()
        for i in range(fpn_num):
            classhead.append(ClassHead(inchannels, anchor_num))
        return classhead

    def _make_bbox_head(self, fpn_num=3, inchannels=64, anchor_num=16):
        bboxhead = nn.ModuleList()
        for i in range(fpn_num):
            bboxhead.append(BboxHead(inchannels, anchor_num))
        return bboxhead

    def _make_landmark_head(self, fpn_num=3, inchannels=64, anchor_num=16):
