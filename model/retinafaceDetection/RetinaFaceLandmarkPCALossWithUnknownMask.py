...
# landmark_pca_feature_size = 6


class RetinaFaceLandmarkPCALossWithUnknownMask(nn.Module):

    def __init__(self, train_param, network_param, pca_param_dict):
        super(RetinaFaceLandmarkPCALossWithUnknownMask, self).__init__()
        self.num_classes = train_param.num_of_class
        self.positive_sample_score_threshold = train_param.pos_sample_score_threshold
        self.neg_pos_ratio = train_param.neg_pos_ratio
        self.variance = train_param.variance
        self.landmark_dim = train_param.landmark_size * 2
        self.useful_key_pys_num = 4
        self.pytorch_device = torch.device("cpu" if train_param.CPU else "cuda")
        ...
        self.ori_img_width = train_param.img_width
        self.ori_img_height = train_param.img_height
        ...
        self.singular_values = pca_param_dict['normalized_singular_values'].to(self.pytorch_device).to(torch.float32)[0:self.pca_feature_size]
    ...
    def forward(self, predictions, priors, targets, unknown_masks):

        #！！加入和先验一样大小的mask
        #！！mask是用于负样本的挖掘，和正样本无关，正样本只和gt匹配分数有关
        ...
        assert loc_data.size(1) == unknown_masks.size(1)
        ...
        landm_pca_weight_t = torch.Tensor(batch_size, num_priors, self.pca_feature_size)
        ...
        #conf_t每个样本索引对应的类别,0,1,..nClass,-1,-1表示只有框，没有关键点
        ...
        # 利用预测值还原特征点
        landm_pca_weight_t = landm_pca_weight_t[pos_idx_pca_weight1].view(-1, self.pca_feature_size)
        landm_pca_weight_p = landm_pca_weight[pos_idx_pca_weight1].view(-1, self.pca_feature_size)
        ...
        loss_weight = torch.sum((landm_pca_weight_p * landm_pca_weight_p) / (singular_values_expanded * singular_values_expanded)) / N1
        # if loss_landm > 1000:
        #     pass
        ...
        #!!通过未知样本再过滤一次
        loss_c[unknown_masks.view(-1, 1)] = 0
        ...
        # focal_loss = focalloss.FocalLoss(gamma=7)(conf_p, targets_weighted)
